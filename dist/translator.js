...
const last_1 = require("./util/last");
class Translator {
    constructor() {
        this.namespace = 'test';
        this.path = ['main'];
    ...
    add(value, newline = true) {
        if (!this.safe) {
            return value;
        }
        let path = last_1.last(this.path);
    ...
    genPath(value = last_1.last(this.path)) {
    ...
    updateWorkingFile(path = last_1.last(this.path), selector = this.currentSelector()) {
        let exec = this.genExecute(path, selector);
        switch (last_1.last(this.blockInfo).type) {
            case 'def':
                break;
            default:
                this.add(exec);
    ...
    updateSelector(raw_selector) {
        ...
        last_1.last(this.blockInfo).selector = selector;
    ...
    currentSelector() {
        return last_1.last(this.blockInfo).selector;
    }
}
